...
!function(w) {
	var l = w.location, b = w.bowljs, libHost, appHost,
		TIMESTAMP = (w.___script___ && w.___script___.TIMESTAMP) ? w.___script___.TIMESTAMP : '20160526';

	if (l.href.indexOf('useSuperLink') !== -1) {
		libHost = appHost = l.host + '/fefiles';
	} else {
		libHost = 'file1.qf.56.itc.cn';
		appHost = 'file2.qf.56.itc.cn';
	}

	b.config({
		libPath: 'http://'+ libHost +'/modjs/lib',
		appPath: 'http://'+ appHost +'/modjs/app',
		charset: 'utf-8',
		map: [
			function(u) {
				u.search = TIMESTAMP;
